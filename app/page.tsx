...
export default async function HomePage() {
  ...
  for (const r of results ?? []) {
    ...
    if (!stats.has(player.id)) {
      stats.set(player.id, { id: player.id, name: player.name, games: 0, rankSum: 0, firsts: 0, fourths: 0, ptSum: 0 })
    }
  ...
  return (
    <main>
      <h1>PT 排行榜</h1>
      <p className="muted">按累计 PT 排名，采用 M.LEAGUE 计分规则。点击玩家查看个人主页。</p>
      <div className="card">
        {leaderboard.length === 0 ? (
          <p className="muted">还没有对局数据。先添加玩家并录入第一场。</p>
        ) : (
          <table>
            <thead><tr><th>排名</th><th>玩家</th><th>PT</th><th>场次</th><th>均顺</th><th>一位率</th><th>四位率</th><th>平均 PT</th></tr></thead>
            <tbody>
